...
#if 0
	#if defined(CONFIG_BOARD_DAPHNE)
		unsigned char i_CTPM_FW0[] = {/*BOE */
			#include "HX83112A_FM_Version_Boe.i"
		};
	#else
		unsigned char i_CTPM_FW0[] = {/*dijing */
			#include "HX83102A_FM_Version_Dijing.i"
		};
		unsigned char i_CTPM_FW1[] = {/*lead */
			#include "HX83102A_FM_Version_Lead.i"
		};
		unsigned char i_CTPM_FW2[] = {/*xinli */
			#include "HX83102A_FM_Version_Xinli.i"
		};
	#endif
#endif
	unsigned char i_CTPM_FW0[] = {/*dijing */
			#include HX_UPGRADE_FW0_APP
		};
#if (HX_GET_VENDOR_ID_NUM >= 2)
	unsigned char i_CTPM_FW1[] = {/*lead */
			#include HX_UPGRADE_FW1_APP
		};
#endif
#if (HX_GET_VENDOR_ID_NUM >= 3)
	unsigned char i_CTPM_FW2[] = {/*xinli */
			#include HX_UPGRADE_FW2_APP
		};
#endif
#if (HX_GET_VENDOR_ID_NUM >= 4)
	unsigned char i_CTPM_FW3[] = {/*xinli */
			#include HX_UPGRADE_FW3_APP
		};
	#endif
#endif
...
#endif
	int g_vendor_id = 0; /*which vendor*/
...
extern void himax_log_touch_event_detail(struct himax_ts_data *ts,
				int x, int y, int w, int loop_i, uint8_t EN_NoiseFilter,
...
int himax_input_register(struct himax_ts_data *ts)
{
	...
	set_bit(KEY_GESTURE_DOUBLEC, ts->input_dev->keybit);
...
static void calcDataSize(uint8_t finger_num)
{
	...
	I("%s: coord_data_size: %d, area_data_size:%d, raw_data_frame_size:%d, raw_data_nframes:%d",
		__func__, ts_data->coord_data_size,
...
#ifdef HX_AUTO_UPDATE_FW
extern int tp_update_firmware_flag;
static int i_update_FW(void)
{
	...
	update_flag = 1;
	tp_update_firmware_flag = 1;
	I("%s: i_fullFileLength = %d\n", __func__, fullFileLength);
#if 0
	#if defined(CONFIG_BOARD_DAPHNE)
		if (g_vendor_id == HX_VENDOR_ID_BOE) {
			ImageBuffer = i_CTPM_FW0;
			fullFileLength = sizeof(i_CTPM_FW0);
			I("HXTP,it's boe\n");
		} else {
			ImageBuffer = i_CTPM_FW0;
			fullFileLength = sizeof(i_CTPM_FW0);
	...
	#else
		if (g_vendor_id == HX_VENDOR_ID_DIJING) {
			ImageBuffer = i_CTPM_FW0;
			fullFileLength = sizeof(i_CTPM_FW0);
			I("HXTP,it's dijing\n");
		} else if (g_vendor_id == HX_VENDOR_ID_LEAD) {
			ImageBuffer = i_CTPM_FW1;
			fullFileLength = sizeof(i_CTPM_FW1);
			I("HXTP,it's lead\n");
		} else if (g_vendor_id == HX_VENDOR_ID_XINLI) {
	...
	#endif
#endif
	if (g_vendor_id == HX_VENDOR_ID_0) {
			ImageBuffer = i_CTPM_FW0;
			fullFileLength = sizeof(i_CTPM_FW0);
			I("HXTP,it's dijing\n");
		}
#if (HX_GET_VENDOR_ID_NUM >= 2)
	else if (g_vendor_id == HX_VENDOR_ID_1) {
			ImageBuffer = i_CTPM_FW1;
			fullFileLength = sizeof(i_CTPM_FW1);
			I("HXTP,it's lead\n");
		}
#endif
#if (HX_GET_VENDOR_ID_NUM >= 3)
	else if (g_vendor_id == HX_VENDOR_ID_2) {
			ImageBuffer = i_CTPM_FW2;
			fullFileLength = sizeof(i_CTPM_FW2);
			I("HXTP,it's app2\n");
		}
#endif
#if (HX_GET_VENDOR_ID_NUM >= 4)
	else if (g_vendor_id == HX_VENDOR_ID_3) {
...
update_retry:
	if (fullFileLength == FW_SIZE_32k) {
		ret = fts_ctpm_fw_upgrade_with_sys_fs_32k(private_ts->client,
		ImageBuffer, fullFileLength, false);
	} else if (fullFileLength == FW_SIZE_60k) {
		ret = fts_ctpm_fw_upgrade_with_sys_fs_60k(private_ts->client,
		ImageBuffer, fullFileLength, false);
	} else if (fullFileLength == FW_SIZE_64k) {
		ret = fts_ctpm_fw_upgrade_with_sys_fs_64k(private_ts->client,
		ImageBuffer, fullFileLength, false);
	} else if (fullFileLength == FW_SIZE_124k) {
		ret = fts_ctpm_fw_upgrade_with_sys_fs_124k(private_ts->client,
		ImageBuffer, fullFileLength, false);
	} else if (fullFileLength == FW_SIZE_128k) {
		ret = fts_ctpm_fw_upgrade_with_sys_fs_128k(private_ts->client,
		ImageBuffer, fullFileLength, false);
	}
	if (ret == 0) {
		upgrade_times++;
...
static void himax_chip_monitor_function(struct work_struct *work) /*for ESD solution*/
{
	...
	if (g_chip_monitor_data->HX_CHIP_POLLING_COUNT >=
		(g_chip_monitor_data->HX_POLLING_TIMES-1)) {
		/*POLLING TIME*/
...
void himax_wake_check_func(void)
{
	...
	case EV_GESTURE_02:
		...
		break;
	case EV_GESTURE_03:
		KEY_EVENT = KEY_GESTURE_LEFT;/*<-*/
		break;
	case EV_GESTURE_04:
		KEY_EVENT = KEY_GESTURE_RIGHT;/*->*/
		break;
	case EV_GESTURE_05:
		KEY_EVENT = KEY_GESTURE_C;/*c */
		break;
	case EV_GESTURE_06:
		KEY_EVENT = KEY_CUST_06;
		break;
	case EV_GESTURE_07:
		KEY_EVENT = KEY_GESTURE_M;/*M*/
		break;
	case EV_GESTURE_08:
		KEY_EVENT = KEY_GESTURE_O;/*O*/
		break;
	case EV_GESTURE_09:
...
int himax_report_data_init(void)
{
	...
	if (hx_touch_data->raw_cnt_rmd != 0x00) { /*more than 4 fingers*/
		hx_touch_data->rawdata_size =
			cal_data_len(hx_touch_data->raw_cnt_rmd,
	...
	if ((ic_data->HX_TX_NUM * ic_data->HX_RX_NUM +
		ic_data->HX_TX_NUM + ic_data->HX_RX_NUM) %
		hx_touch_data->rawdata_size == 0)
		hx_touch_data->rawdata_frame_size =
		(ic_data->HX_TX_NUM * ic_data->HX_RX_NUM + ic_data->HX_TX_NUM + ic_data->HX_RX_NUM) /
			hx_touch_data->rawdata_size;
	else
		hx_touch_data->rawdata_frame_size =
			(ic_data->HX_TX_NUM * ic_data->HX_RX_NUM + ic_data->HX_TX_NUM + ic_data->HX_RX_NUM) /
	...
	I("%s: ic_data->HX_MAX_PT:%d,", __func__, ic_data->HX_MAX_PT);
	I("hx_raw_cnt_max:%d, hx_raw_cnt_rmd:%d", hx_touch_data->raw_cnt_max, hx_touch_data->raw_cnt_rmd);
	...
	hx_touch_data->hx_rawdata_buf =
		kzalloc(sizeof(uint8_t)*(hx_touch_data->touch_all_size - hx_touch_data->touch_info_size), GFP_KERNEL);
	if (hx_touch_data->hx_rawdata_buf == NULL)
		goto mem_alloc_fail;
#endif
...
void himax_report_points(struct himax_ts_data *ts)
{
	...
	if (hx_point_num != 0) {
		...
		for (loop_i = 0; loop_i < ts->nFinger_support; loop_i++) {
			...
			*/
			/* smart area function regulate start*/
			if (w < 27) {
			  w = w / 3 + 1;
			} else {
			  w = w / 3 + 3;
			}
			/* smart area function regulate end*/
			if (x >= 0 && x <= ts->pdata->abs_x_max && y >= 0 && y <= ts->pdata->abs_y_max) {
				hx_touch_data->finger_num--;
				if ((ts->debug_log_level & BIT(3)) > 0) {
					himax_log_touch_event_detail(ts, x, y, w, loop_i,
						EN_NoiseFilter, HX_FINGER_ON, old_finger);
				}
#ifndef	HX_PROTOCOL_A
				...
				if ((ts->debug_log_level & BIT(3)) > 0) {
					himax_log_touch_event_detail(ts, x, y, w, loop_i,
						Last_EN_NoiseFilter, HX_FINGER_LEAVE, old_finger);
				}
			}
		}
#ifndef	HX_PROTOCOL_A
	...
	else if (hx_touch_data->hx_coord_buf[HX_TOUCH_INFO_POINT_CNT + 3] != 0) {
#if defined(HX_PALM_REPORT)
...
int himax_chip_common_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	struct himax_ts_data *ts;
	struct himax_i2c_platform_data *pdata;
	uint32_t tp_id0, tp_id1;/*add by zte for read lcd hardware id*/

#ifdef HX_AUTO_UPDATE_FW
	bool normal_boot = false;/*not in normal status*/
	const char *str = (const char *)saved_command_line;
	/*const char *ret_boot = strnstr(str, "androidboot.mode=normal", sizeof("androidboot.mode=normal"));*/
	const char *ret_boot = strstr(str, "androidboot.mode=normal");
#endif

	D("%s:enter himax driver\n", __func__);
...
#endif
	I(" himax begin to power config\n");
	himax_gpio_power_config(ts->client, pdata);
	I(" himax end power config\n");
#ifndef CONFIG_OF
	if (pdata->power) {
		ret = pdata->power(1);
		if (ret < 0) {
...
#if 0
	#if defined(CONFIG_BOARD_DAPHNE)
		tp_id0 = gpio_get_value(125);
		tp_id1 = gpio_get_value(92);
		I("HXTP,lcd_id0=%d, lcd_id1=%d\n", tp_id0, tp_id1);
		if ((tp_id0 == 0) && (tp_id1 == 0)) {/*boe*/
			g_vendor_id = HX_VENDOR_ID_BOE;
			I("HXTP,it's boe\n");
		} else {/*boe*/
			g_vendor_id = HX_VENDOR_ID_BOE;
			I("HXTP,it's boe\n");
	...
	#else
		/* identify vendor id */
		tp_id0 = gpio_get_value(47);
		tp_id1 = gpio_get_value(50);
		I("HXTP,lcd_id0=%d, lcd_id1=%d\n", tp_id0, tp_id1);

		if ((tp_id0 == 0) && (tp_id1 == 1)) {/*dijing*/
			g_vendor_id = HX_VENDOR_ID_DIJING;
			I("HXTP,it's dijing\n");
		} else if ((tp_id0 == 0) && (tp_id1 == 0)) {/*lead*/
			g_vendor_id = HX_VENDOR_ID_LEAD;
			I("HXTP,it's lead\n");
		} else if ((tp_id0 == 1) && (tp_id1 == 0)) {/*xinli*/
			g_vendor_id = HX_VENDOR_ID_XINLI;
			I("HXTP,it's xinli\n");
		}
	#endif
#endif
	/* identify vendor id */
	if (gpio_is_valid(pdata->gpio_id0) && gpio_is_valid(pdata->gpio_id1)) {
		...
		tp_id0 = 0;
		tp_id1 = 0;
		I("HXTP,lcd_id0 &  lcd_id1 are all invalid\n");
	}

		if ((tp_id0 == 0) && (tp_id1 == 1)) {/*dijing*/
			g_vendor_id = HX_VENDOR_ID_0;
			I("HXTP,it's VENDOR_ID_0\n");
		} else if ((tp_id0 == 0) && (tp_id1 == 0)) {/*lead*/
			g_vendor_id = HX_VENDOR_ID_1;
			I("HXTP,it's VENDOR_ID_1\n");
		} else if ((tp_id0 == 1) && (tp_id1 == 0)) {/*unknown*/
			g_vendor_id = HX_VENDOR_ID_2;
			I("HXTP,it's VENDOR_ID_2\n");
		} else  if ((tp_id0 == 1) && (tp_id1 == 1)) {/*unknown*/
			g_vendor_id = HX_VENDOR_ID_3;
			I("HXTP,it's VENDOR_ID_3\n");
		}
	...
	queue_delayed_work(ts->himax_chip_monitor_wq,
		&ts->himax_chip_monitor, g_chip_monitor_data->HX_POLLING_TIMER*HZ);
#endif
...
#endif
	lcd_tp_rst_vdd_sleep_keephigh_for_hx83112a = true;
	return 0;
...
int himax_chip_common_suspend(struct himax_ts_data *ts)
{
	...
	if (0) {
		...
		if (ret) {
			E("Failed to power off hardware\n");
		}
	}
	I("%s: %d---\n", __func__, ts->pdata->powerOff3V3);
...
int himax_chip_common_resume(struct himax_ts_data *ts)
{
	...
	if (ts->pdata->powerOff3V3 && ts->pdata->power) {
		E("enter power on hardware in resume\n");
		ts->pdata->power(1);
	...
	himax_int_enable(ts->client->irq, 1);
	I("resume power on init\n");
	/*himax_power_on_init(client);*/
	I("resume report data init\n");
	/*himax_report_data_init();*/
#ifdef HX_CHIP_STATUS_MONITOR
	g_chip_monitor_data->HX_CHIP_POLLING_COUNT = 0;
	g_chip_monitor_data->HX_CHIP_MONITOR_EN = 1;
	queue_delayed_work(ts->himax_chip_monitor_wq, &ts->himax_chip_monitor,
		g_chip_monitor_data->HX_POLLING_TIMER*HZ); /*for ESD solution*/
#endif
	I("%s: END\n", __func__);
	return 0;
}
