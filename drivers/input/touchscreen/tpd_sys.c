					 ...
					 char __user *buffer, size_t count, loff_t *offset)
{
	...
	if (tp_reg_rw.len < 0) {
		...
		if (tp_reg_rw.type == REG_OP_READ) {
			if (tp_reg_rw.res == 0) {
				count = snprintf(buf, PAGE_SIZE, "Read reg 0x%02X: 0x%02X\n",
					tp_reg_rw.reg, tp_reg_rw.val);
			} else {
				count =
				    snprintf(buf, PAGE_SIZE, "Read reg 0x%02X failed, ret: %d\n", tp_reg_rw.reg,
					     tp_reg_rw.res);
			}
		} else {
			if (tp_reg_rw.res == 0) {
				count =
				    snprintf(buf, PAGE_SIZE, "Write reg 0x%02X, 0x%02X success\n", tp_reg_rw.reg,
					     tp_reg_rw.val);
			} else {
				count =
				    snprintf(buf, PAGE_SIZE, "Write reg 0x%02X failed, ret: %d\n", tp_reg_rw.reg,
					     tp_reg_rw.res);
			}
		}
	} else {
		if (tp_reg_rw.type == REG_OP_READ) {
			...
			count =
			    snprintf(buf, PAGE_SIZE, "Read Reg: [0x%02X]-[0x%02X]\n", tp_reg_rw.reg,
			...
			if (tp_reg_rw.res) {
				...
				if (tp_reg_rw.opbuf) {
					for (i = 0; i < tp_reg_rw.len; i++) {
						count +=
						    snprintf(buf + count, PAGE_SIZE, "0x%02X ", tp_reg_rw.opbuf[i]);
					}
			...
			length = (length < 0) ? 0 : length;
			count =
			    snprintf(buf, PAGE_SIZE, "Write Reg: [0x%02X]-[0x%02X]\n", tp_reg_rw.reg,
				     tp_reg_rw.reg + length);
			count += snprintf(buf + count, PAGE_SIZE, "Write Data: ");
			if (tp_reg_rw.opbuf) {
				for (i = 0; i < tp_reg_rw.len; i++) {
					count += snprintf(buf + count, PAGE_SIZE, "0x%02X ", tp_reg_rw.opbuf[i]);
				}
				count += snprintf(buf + count, PAGE_SIZE, "\n");
			}
			if (tp_reg_rw.res) {
				count +=
				    snprintf(buf + count, PAGE_SIZE, "Result: failed, ret: %d\n", tp_reg_rw.res);
			} else {
	...
	char __user *buffer, size_t count, loff_t *offset)
{
	...
	if (cdev->ic_tpinfo.display_ver)
		len += snprintf(buffer_tpd+len, PAGE_SIZE, "Display version:0x%x\n", cdev->ic_tpinfo.display_ver);
